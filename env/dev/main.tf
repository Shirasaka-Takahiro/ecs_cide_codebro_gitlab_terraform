...
module "ec2" {
  ...
  ami               = var.ami
  public_subnet_ids = module.network.public_subnet_ids
  ec2_http_sg_id    = module.ec2_http_sg.security_group_id
  ec2_https_sg_id   = module.ec2_https_sg.security_group_id
...
module "domain" {
  ...
  public_ip    = element(module.ec2.public_ip, 0) #tuple の 0 番目を取り出して文字列として渡す
}
